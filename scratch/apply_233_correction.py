...
src_dir = r"d:\Kuliah\KP\project_bps_tasik\outputs\Ekstraksi_Hal_74_sd_74"
...
if csv_file:
    ...
    with engine.connect() as conn:
        # Cari Table ID
        row = conn.execute(text("SELECT id FROM extracted_tables WHERE document_id = 58 AND table_name LIKE 'Tabel 2.3.3%'")).fetchone()
        if row:
            table_id = row[0]
            ...
            print("Tabel 2.3.3 tidak ditemukan di database.")
else:
