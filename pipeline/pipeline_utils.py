...
def fix_doubled_text(text):
    ...
    text = str(text)
    # Jangan bersihkan jika string hanya berupa angka/numeric (menghindari merusak 11, 22, 11.11, dll)
...
def parse_indonesian_number(val_str):
    ...
    # Case 3: Only dot present
        ...
        if val_str.count('.') > 1:
            parts = val_str.split('.')
            if len(parts[-1]) in [1, 2] and all(len(p) <= 3 for p in parts[:-1]):
            ...
            if parts[0] != '0' and len(parts) == 2 and len(parts[1]) == 3:
                # Thousands separator (e.g. 74.433, 3.776) — tapi bukan 0.xxx (desimal)
                val_str = val_str.replace('.', '')
            # selain itu: desimal (0.730, 1.17) — biarkan titik tetap
...
INDO_SAFE_WORDS = {
    ...
    'hotel', 'hotels', 'total', 'totals', 'unit', 'units', 'status', 'area',
    'areas', 'bus', 'buses', 'gas', 'internet', 'internets', 'target', 'villa', 'villas'
}
