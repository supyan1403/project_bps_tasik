...
def clean_chapter_title(title: str) -> str:
    ...
    title = re.sub(r'([a-zA-Z])[1-9](?![0-9])', r'\1', title)
    title = re.sub(r'\bT\s+T\b', '', title, flags=re.IGNORECASE)
    title = re.sub(r'^T(?=[A-Z][a-z])', '', title) # e.g. TPerdagangan -> Perdagangan, TPariwisata -> Pariwisata
    replacements = [
        (r'\bKetenagakerjaabn\b', 'Ketenagakerjaan'),
        (r'\bKetenagakerjaanb\b', 'Ketenagakerjaan'),
        (r'\bDanm\b', 'dan'),
        (r'\bDans\b', 'dan'),
        (r'\bKeksejahteraan\b', 'Kesejahteraan'),
        (r'\bKesejahteraana\b', 'Kesejahteraan'),
        (r'\bKesejahteraasn\b', 'Kesejahteraan'),
        (r'\bPertaniaan\b', 'Pertanian'),
        (r'\bKehutanana\b', 'Kehutanan'),
        (r'\bPeternakaan\b', 'Peternakan'),
        (r'\bIndsustri\b', 'Industri'),
        (r'\bPertatmbangan\b', 'Pertambangan'),
        (r'\bDaanl\b', 'dan'),
        (r'\bPariwisatas\b', 'Pariwisata'),
        (r'\bTransportsasii\b', 'Transportasi'),
        (r'\bTranspotrtasi\b', 'Transportasi'),
        (r'\bPerbantkan\b', 'Perbankan'),
        (r'\bPerb:ankan\b', 'Perbankan'),
        (r'\bKotperasi\b', 'Koperasi'),
        (r'\bPesngeluaran\b', 'Pengeluaran'),
        (r'\bPpengeluaran\b', 'Pengeluaran'),
        (r'\bKonspumsi\b', 'Konsumsi'),
        (r'\bKabkupaten\b', 'Kabupaten'),
        (r'\bPemerintahian\b', 'Pemerintahan'),
    ]
    for pat, rep in replacements:
        title = re.sub(pat, rep, title, flags=re.IGNORECASE)
    title = re.sub(r'\s+', ' ', title).strip()
    return title_case_indonesian(title)
...
def parse_bps_toc(pdf_path):
    with pdfplumber.open(pdf_path) as pdf:
        ...
        # Re-group tables by filtering out outliers using IQR
        ...
        # Calculate cover start for each chapter
        starts = []
        for idx, c in enumerate(chapters):
            num = c["num"]
            raw_s = c["raw_start"]
            
            f_tp = min(filtered_table_pages[num]) if (num in filtered_table_pages and filtered_table_pages[num]) else None
            
            if num == 1:
                if f_tp:
                    c_start = max(30, f_tp + offset - 6)
                else:
                    c_start = raw_s + offset
            else:
                prev_start = starts[-1] if starts else 0
                calc_start = raw_s + offset
                is_valid = False
                if calc_start > prev_start:
                    if f_tp is None:
                        is_valid = True
                    elif 0 <= (f_tp + offset - calc_start) <= 25:
                        is_valid = True
                        
                if is_valid:
                    c_start = calc_start
                elif f_tp:
                    c_start = max(prev_start + 1, f_tp + offset - 6)
                else:
                    c_start = prev_start + 10
            starts.append(c_start)

        # Build babs where end_page is next_start - 1 (seamless, no cut off tables)
        babs = []
        for idx, c in enumerate(chapters):
            c_start = starts[idx]
            if idx + 1 < len(chapters):
                c_end = starts[idx + 1] - 1
            else:
                c_end = total_pages
            ...
            babs.append({
                "num": c["num"],
                "title": c["title"],
                "start_page": max(1, c_start),
                "end_page": min(total_pages, c_end)
            })
