...
# 1. Salin CSV hasil ekstraksi terbaru tahun 2023 ke folder web
src_dir = r"d:\Kuliah\KP\project_bps_tasik\outputs\Ekstraksi_Hal_79_sd_84"
dst_dir = r"d:\Kuliah\KP\project_bps_tasik\backend\hasil_ekstraksi_web\doc_69\Ekstraksi_Hal_68_sd_90"

os.makedirs(dst_dir, exist_ok=True)

csv_file = None
for f in os.listdir(src_dir):
    if f.startswith("Tabel 3.1.1") and f.endswith(".csv"):
        csv_file = f
        break
...
if csv_file:
    ...
    with engine.connect() as conn:
        # Cari Table ID
        row = conn.execute(text("SELECT id FROM extracted_tables WHERE document_id = 69 AND table_name LIKE 'Tabel 3.1.1%'")).fetchone()
        if row:
            table_id = row[0]
            ...
            if raw_rows:
                ...
                for rec in records:
                    ...
                    # PostgreSQL Native JSONB insert
                    # Ganti tanda petik tunggal agar aman dalam query SQL manual
            ...
            print("Tabel 3.1.1 tidak ditemukan di database.")
else:
