...
if [ -d "$PROJECT_DIR/frontend/static" ]; then
    STATIC_ROOT="$PROJECT_DIR/frontend/static"
else
...
# Backward-compatible symlink untuk Nginx lama yang masih membaca /backend/static
if [ ! -e "$PROJECT_DIR/backend/static" ] && [ -d "$PROJECT_DIR/frontend/static" ]; then
    ln -s "$PROJECT_DIR/frontend/static" "$PROJECT_DIR/backend/static" 2>/dev/null && print_ok "Backward-compatible static symlink created." || true
fi
...
# Cari 502.html
SOURCE_502="$PROJECT_DIR/frontend/static/502.html"
if [ ! -f "$SOURCE_502" ]; then
    SOURCE_502="$PROJECT_DIR/frontend/templates/502.html"
fi
if [ ! -f "$SOURCE_502" ]; then
