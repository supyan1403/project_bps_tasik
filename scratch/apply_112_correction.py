...
src_dir = r"d:\Kuliah\KP\project_bps_tasik\outputs\Ekstraksi_Hal_51_sd_52"
dst_dir = r"d:\Kuliah\KP\project_bps_tasik\backend\hasil_ekstraksi_web\doc_58\Ekstraksi_Hal_39_sd_56"
...
if csv_file:
    ...
    with engine.connect() as conn:
        # Cari Table ID
        row = conn.execute(text("SELECT id FROM extracted_tables WHERE document_id = 58 AND table_name LIKE 'Tabel 1.1.2%'")).fetchone()
        if row:
            table_id = row[0]
            ...
            print("Tabel 1.1.2 tidak ditemukan di database.")
else:
