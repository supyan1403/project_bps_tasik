...
try:
    ...
    if not doc:
        print("Document 55 not found.")
        sys.exit(1)
        
    base_backend = os.path.abspath(os.path.join(os.path.dirname(__file__), "../backend"))
    output_path = os.path.join(base_backend, "hasil_ekstraksi_web", f"doc_{doc_id}")
    print(f"Scanning folder: {output_path}")
    ...
    count = 0
    # Walk through the doc folder to find Ekstraksi_Hal folders
    for sub in os.listdir(output_path):
        ...
        if os.path.isdir(sub_path) and sub.startswith("Ekstraksi_Hal_"):
            print(f"Found extraction folder: {sub}")
            ...
            for root, _, files in os.walk(sub_path):
                for file in files:
                    if file.endswith(".csv"):
                        ...
                        # Use relative path in database to match main.py format
    ...
    db.commit()
    print(f"Successfully registered {count} tables in database for Document 55.")
